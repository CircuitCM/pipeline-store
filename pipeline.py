...
import inspect
from ._async import link_robust
...
class Pipeline(BaseModel):
    ...
    @classmethod
    def parse_obj(cls, obj: dict,_depth: int = 0) -> 'Pipeline':
        # Parse the dictionary to get a basic Pipeline instance
        instance = super().parse_obj(obj)
        for step in instance.steps:
            step.args = [cls._parse_nested_pipeline(value, _depth + 1) if isinstance(value, dict) else value for value in step.args]
            step.kwargs = {key: cls._parse_nested_pipeline(value, _depth + 1) if isinstance(value, dict) else value for key, value in step.kwargs.items()}
        return instance
    ...
    def _parse_nested_pipeline(cls, value: dict, _depth: int) -> Any:
        ...
        try:
            return cls.parse_obj(value, _depth)
    ...
    async def __call__(self, *args, **kwargs):
        ...
        for step in self.steps:
            ...
            p_or_k = [p.name for p in inspect.signature(func).parameters.values() if p.kind == inspect.Parameter.POSITIONAL_OR_KEYWORD] #for now assume all callables don't have any funky restrictions.
            kwargs ={k:v for k,v in zip(p_or_k,args)}
            #print(kwargs)
            orr=step.kwargs|glvars
            kwargs|={k:orr[k] for k in p_or_k[len(args):] if k in orr} #can be changed to override certain args instead of other way around if thats ever a need.
            #print(kwargs)
            if len(kwargs)>0:
                #first evaluate arguments if they exist.
                argval=await link_robust(*kwargs.values())#should only eval pipelines as they are serializable unless a callable given in glvars.
                kwargs={k:res for k,res in zip(kwargs.keys(),argval)}
            if not func:
                raise ValueError(f"Function {func_name} not found")
            if asyncio.iscoroutinefunction(
                    func):  # to tell python a sync def is a courotine function func._is_coroutine=aio.coroutines._is_coroutine
